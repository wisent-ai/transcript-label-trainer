...
pub(crate) const DOCS_INDEX: &[u8] = include_bytes!("../../docs/index.html");
...
pub(crate) fn handle(mut request: Request, authority: &str, origin: &str, token: &str) -> Result<()> {
    if request
        .remote_addr()
...
pub(crate) fn state() -> Result<Value> {
    Ok(json!({
        "ok": true,
...
pub(crate) fn empty_report(rejected: usize, conflicting: usize) -> Value {
    json!({
        "imported": 0,
        "unchanged": 0,
